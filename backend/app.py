...
def reset_password(request: ResetPasswordRequest):
    ...
    print(f"\nPassword reset completed for user: {user['username']} (ID: {user_id})")
...
def logout(request: Request):
    ...
    # blacklist_token not yet implemented — just return success
...
def get_last_login_activity(
    current_user: dict = Depends(get_current_user),
    _: None = Depends(check_inactivity)
):
...
def sponsor_catalog(current_user: dict = Depends(get_current_user)):
    ...
    try:
        cursor.execute(
            "SELECT * FROM SponsorCatalog WHERE sponsor_user_id = %s ORDER BY created_at DESC",
            (sponsor_id,)
        )
...
def add_product_to_catalog(product: dict, current_user=Depends(get_current_user)):
...
def delete_product_from_catalog(item_id: str, current_user=Depends(get_current_user)):
    if current_user["role"] != "sponsor":
        raise HTTPException(status_code=403, detail="Not authorized")
    try:
...
def publish_catalog(current_user=Depends(get_current_user)):
    ...
    try:
        cursor.execute(
            ...
            SET is_published = TRUE, is_draft = FALSE, is_active = TRUE
            WHERE sponsor_user_id = %s AND is_draft = TRUE
            """,
            (sponsor_id,)
        )
        conn.commit()
...
def disable_catalog_item(item_id: str, current_user=Depends(get_current_user)):
    if current_user["role"] != "sponsor":
        raise HTTPException(status_code=403, detail="Not authorized")
    conn = get_connection()
    cursor = conn.cursor()
    try:
        cursor.execute(
            "UPDATE SponsorCatalog SET is_active = FALSE WHERE item_id = %s AND sponsor_user_id = %s",
...
def enable_catalog_item(item_id: str, current_user=Depends(get_current_user)):
    if current_user["role"] != "sponsor":
        raise HTTPException(status_code=403, detail="Not authorized")
    conn = get_connection()
    cursor = conn.cursor()
    try:
        cursor.execute(
            "UPDATE SponsorCatalog SET is_active = TRUE WHERE item_id = %s AND sponsor_user_id = %s",
            (item_id, current_user["user_id"])
        )
        conn.commit()
...
def get_driver_catalog(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        cursor.execute(
            ...
            FROM SponsorCatalog
            WHERE sponsor_user_id = %s AND is_published = TRUE AND is_active = TRUE
            ORDER BY title ASC
            """,
            (row["sponsor_user_id"],)
        )
...
def sponsor_purchase_for_driver(body: dict, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        current_points = driver_row["total_points"] or 0
        cursor.execute(
            "SELECT item_id, title, points_cost, stock_quantity FROM SponsorCatalog WHERE item_id = %s AND sponsor_user_id = %s AND is_active = TRUE AND is_published = TRUE",
            (item_id, sponsor_id)
        )
        item = cursor.fetchone()
        if not item:
        ...
        if current_points < item["points_cost"]:
            ...
            raise HTTPException(status_code=400, detail=f"Insufficient driver points. '{item['title']}' costs {item['points_cost']} pts; driver has {current_points} pts (needs {needed} more).")
        if item["stock_quantity"] <= 0:
            raise HTTPException(status_code=400, detail="This item is out of stock.")
        cursor.execute(
        ...
        cursor.execute(
            "INSERT INTO Orders (driver_user_id, sponsor_user_id, item_id, item_title, points_cost, status, created_at, updated_at) VALUES (%s, %s, %s, %s, %s, 'pending', %s, %s)",
            (driver_id, sponsor_id, item_id, item["title"], item["points_cost"], now, now)
        )
        conn.commit()
        ...
        cursor.execute("SELECT stock_quantity FROM SponsorCatalog WHERE item_id = %s AND sponsor_user_id = %s", (item_id, sponsor_id))
        new_stock = cursor.fetchone()["stock_quantity"]
        return {"message": f"Purchased '{item['title']}' for driver #{driver_id}", "points_spent": item["points_cost"], "driver_new_points_balance": new_balance, "remaining_stock": new_stock}
    except HTTPException:
        conn.rollback()
        raise
    except Exception:
...
def get_driver_orders(current_user: dict = Depends(get_current_user)):
    ...
    try:
        cursor.execute(
            "SELECT order_id, item_id, item_title, points_cost, status, created_at, updated_at FROM Orders WHERE driver_user_id = %s ORDER BY created_at DESC",
            (current_user["user_id"],)
        )
...
def get_sponsor_orders(driver_name: str = "", current_user: dict = Depends(get_current_user)):
    if current_user["role"] != "sponsor":
        raise HTTPException(status_code=403, detail="Sponsor access required")
    sponsor_id = current_user["user_id"]
    conn = get_connection()
    ...
    try:
        cursor.execute(
            """
            SELECT o.order_id, o.item_id, o.item_title, o.points_cost, o.status,
                   o.created_at, o.updated_at, u.username,
...
def get_sponsor_error_logs(limit: int = 100, current_user: dict = Depends(get_current_user)):
    if current_user["role"] != "sponsor":
        raise HTTPException(status_code=403, detail="Sponsor access required")
    sponsor_id = current_user["user_id"]
    conn = get_connection()
    ...
    try:
        cursor.execute(
            "SELECT error_id, occurred_at, operation, endpoint, error_message, status_code, request_id FROM APIErrorLog WHERE sponsor_id = %s OR sponsor_id IS NULL ORDER BY occurred_at DESC LIMIT %s",
...
def get_admin_error_logs(limit: int = 200, current_user: dict = Depends(get_current_user)):
